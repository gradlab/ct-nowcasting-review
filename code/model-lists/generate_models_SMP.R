...
## First generate MGB model list
source("./code/model-lists/generate_models_MGB.R")

## Then identify main model version
models <- models %>% filter(plot_label == main_mdl)

## Then add TT-split version
...
#   test_data = list(NULL), 
#   plot_label = list(c("dmskw_", "dmskw_a_", "dmsk_", "dmskv_", "dmsdw_", "dm_")), 
#   daily=TRUE) %>% 
...
# remove(models_basic, models_alt_dv, models_subgroup, models_comb, models_tt)
# 
# models <- models %>% slice(1, 11)